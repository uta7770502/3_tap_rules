...
test('real API boundary: history, image, errors, validation',async()=>{
 ...
 r=res(); await handler({method:'POST',headers:{},body:{messages}},r); assert.equal(r.code,200); assert.equal(payload.input.length,3); assert.equal(payload.input[0].content[1].type,'input_image'); assert.equal(payload.input[2].content[0].text,messages[2].text); assert.equal(payload.store,false); assert.match(payload.instructions,/10\.1a/); assert.match(payload.instructions,/ダブルヒット/); assert.ok(payload.instructions.includes(require('../competition_rules.json')[249].title)); assert.equal(payload.tool_choice,'required');
