...
(function (root) {
  ...
  const guides = {
    ...
    'bunker-abnormal': { image: 'bunker-water', title: 'バンカー内の一時的な水', label: 'バンカー内と外で、罰が異なる', steps: ['無罰で救済を受ける場合は、バンカー内で救済エリアを決める。', '完全な救済ができない場合は、最大限の救済が得られる地点を確認する。', 'バンカー外を選ぶ場合は、1罰打で後方線上の救済を受ける。'] },
    'bunker-unplayable': { image: 'bunker-unplayable', title: 'バンカーでアンプレヤブル', label: '外への後方線上救済は2罰打', steps: ['1罰打で、前の場所から打ち直すことができる。', '1罰打の後方線上・ラテラル救済では、バンカー内にドロップする。', 'バンカー外の後方線上救済を選ぶ場合は、合計2罰打。'] },
    ...
    'local-e5': { image: 'local-e5', title: 'E-5採用時の前進救済', label: 'ローカルルールの採用確認が先', steps: ['競技・コースでローカルルールE-5が採用されているか確認する。', '球の基準点とフェアウェイの基準点から、規定の救済エリアを特定する。', '適用できる場合に限り、2罰打でその救済エリアへドロップする。'] },
    ...
    replace: { image: 'replace-action', title: '球を元の箇所へリプレース', label: 'ドロップせず、元の箇所へ置く', steps: ['元の箇所を確認する。分からない場合は推定する。', '規則に従い、球をその箇所に置いてリプレースする。', '球が動いた原因・拾い上げた理由に応じた罰を、本文で確認する。'] },
    'drop-method': { image: 'drop-sequence', title: '正しいドロップの方法', label: '立った状態の、膝の高さから', steps: ['適用する規則に従って、救済エリアを決める。', 'プレーヤー本人が、膝の高さから球を真下に落とす。', '球が救済エリアに落ち、その中に止まったことを確認する。'] },
  ...
  const competition = {
    ...
    'bunker-abnormal':[69], 'bunker-unplayable':[74,191], 'ob-lost':[15,39], 'local-e5':[122,215],
    replace:[63,89,177], 'drop-method':[11,64,96,174,178,247], redrop:[65]
  };
  ...
  function render(container, rule, mode) {
    ...
    scene.append(image);
    if (guide.image === 'drop-sequence') {
      ['① 基準点と救済エリアを決める','② 膝の高さから真下へ','③ エリア内に止まった球を使用'].forEach((text,i)=>{
        const label=document.createElement('span'); label.className='sequence-label sequence-'+i; label.textContent=text; scene.append(label);
      });
      const area=document.createElement('span'); area.className='sequence-area'; area.setAttribute('aria-hidden','true'); scene.append(area);
    }
